...
import { ShoppingCart, Heart, Flame, ArrowLeft, SlidersHorizontal, LayoutGrid, Search } from 'lucide-react';

interface SearchSearchParams {
  q?: string;
}

export default function SearchPage({ searchParams }: { searchParams: Promise<SearchSearchParams> }) {
  ...
  // Search logic: matches name or category (case-insensitive)
    ...
    <div className="min-h-screen bg-gray-50 pb-12">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 pt-6">
        {!isMounted ? (
          ...
          <div className="py-20 flex flex-col items-center justify-center text-gray-400 bg-white rounded-2xl border border-gray-100 px-6 text-center">
            <Search size={52} className="mb-4 text-gray-300" />
            <p className="font-semibold text-lg text-gray-600">No results found {query && `for "${query}"`}</p>
            <p className="text-sm text-gray-500 mt-1 mb-6 max-w-md">
              We couldn't find any products matching your search term. Check the spelling or try searching for another product or category (e.g., Electronics, Fashion).
            </p>
